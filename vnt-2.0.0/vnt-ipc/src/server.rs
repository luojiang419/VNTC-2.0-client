...
use crate::message::{
    ...
    ClientRouteList, IpcRequest, IpcResponse, PacketLoss, Route, ServerInfo, TrafficInfoItem,
    TrafficInfoList,
};
use anyhow::bail;
...
async fn handle_connection(stream: TcpStream, vnt_api: VntApi) -> anyhow::Result<()> {
    ...
    if let Some(Ok(message)) = framed.next().await {
        ...
        let rs = match cmd {
            ...
            IpcCmd::TrafficList(_) => {
                let traffic_list = all_traffic(&vnt_api);
...
fn all_traffic(vnt_api: &VntApi) -> TrafficInfoList {
    let items = vnt_api
        .all_traffic_info()
        .into_iter()
        .map(|item| TrafficInfoItem {
            ip: item.ip.into(),
            tx_bytes: item.tx_bytes,
            rx_bytes: item.rx_bytes,
        })
        .collect();
    TrafficInfoList { items }
}
