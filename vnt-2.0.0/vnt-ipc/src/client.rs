...
pub async fn run_client(cmd: IpcCmd, port: Option<u16>) -> anyhow::Result<()> {
        ...
        .context("response payload is empty")?
    {
        ...
        ResponsePayload::TrafficList(traffic_list) => {
            print_traffic_list(traffic_list)?;
        }
    };
    Ok(())
}
...
pub fn print_traffic_list(traffic_list: TrafficInfoList) -> anyhow::Result<()> {
    println!("\n--- Traffic List ({}) ---", traffic_list.items.len());

    let table = traffic_list
        .items
        .iter()
        .map(|item| {
            vec![
                Ipv4Addr::from(item.ip).to_string().cell(),
                item.tx_bytes.to_string().cell(),
                item.rx_bytes.to_string().cell(),
            ]
        })
        .table()
        .title(vec![
            "IP".cell().bold(true),
            "TX Bytes".cell().bold(true),
            "RX Bytes".cell().bold(true),
        ]);
